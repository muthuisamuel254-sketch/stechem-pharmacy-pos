...
export default function POSPage() {
  ...
  const handleAdd = (medicineId: string) => {
    ...
    addToCart({
      medicineId, batchId: batch.id, name: med.name, batchNumber: batch.batchNumber,
      quantity: 1, unitPrice: med.sellingPrice, unit: med.unit, expiryDate: batch.expiryDate,
    });
  };

  const handleCheckout = () => {
    if (cart.length === 0) return;
    const sale = completeSale({ paymentMethod, discount, customerName: customerName || undefined });
    if (sale) {
      setLastSale(sale);
      setShowReceipt(true);
      setDiscount(0);
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <div className="lg:col-span-3 space-y-4">
          <div className="relative">
            ...
            <Input placeholder="Search by name, generic, barcode or category..." className="pl-10 h-12 text-base" value={search} onChange={(e) => setSearch(e.target.value)} autoFocus />
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((med) => {
              const stock = getStockForMedicine(med.id);
              return (
                <button key={med.id} onClick={() => handleAdd(med.id)} disabled={stock === 0}
                  className="group rounded-xl border border-slate-200 bg-white p-4 text-left transition hover:border-teal-300 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-medium text-slate-900 truncate group-hover:text-teal-700">{med.name}</p>
              ...
              {cart.length === 0 ? (
                ...
                <>
                  <div className="max-h-64 space-y-3 overflow-y-auto pr-1">
                    {cart.map((item) => (
                      <div key={`${item.medicineId}-${item.batchId}`} className="rounded-lg border border-slate-100 p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                        ...
                        <div className="mt-2 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <button onClick={() => updateCartQty(item.medicineId, item.batchId, item.quantity - 1)} className="flex h-7 w-7 items-center justify-center rounded border border-slate-200 hover:bg-slate-50"><Minus className="h-3 w-3" /></button>
                            <span className="w-8 text-center text-sm font-medium">{item.quantity}</span>
                            <button onClick={() => updateCartQty(item.medicineId, item.batchId, item.quantity + 1)} className="flex h-7 w-7 items-center justify-center rounded border border-slate-200 hover:bg-slate-50"><Plus className="h-3 w-3" /></button>
                          </div>
                  ...
                  <div className="space-y-1 border-t border-slate-100 pt-3 text-sm">
                    <div className="flex justify-between text-slate-600"><span>Subtotal</span><span>{formatCurrency(subtotal, settings.currency)}</span></div>
                    ...
                    <div className="flex justify-between text-slate-600"><span>Tax ({settings.taxRate}%)</span><span>{formatCurrency(tax, settings.currency)}</span></div>
                    <div className="flex justify-between text-lg font-bold text-slate-900 pt-1"><span>Total</span><span>{formatCurrency(total, settings.currency)}</span></div>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {([{ id: "cash", label: "Cash", icon: Banknote }, { id: "card", label: "Card", icon: CreditCard }, { id: "mobile", label: "Mobile", icon: Smartphone }] as const).map((m) => (
                      <button key={m.id} onClick={() => setPaymentMethod(m.id)}
                        className={`flex flex-col items-center gap-1 rounded-lg border py-2 text-xs font-medium transition ${paymentMethod === m.id ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
            ...
            <div className="p-5 space-y-4 text-sm">
              ...
              <div className="border-y border-dashed border-slate-200 py-3 space-y-1">
                ...
                <div className="flex justify-between"><span className="text-slate-500">Payment</span><span className="capitalize">{lastSale.paymentMethod}</span></div>
              </div>
              ...
              <div className="border-t border-slate-200 pt-3 space-y-1">
                ...
                <div className="flex justify-between text-slate-600"><span>Tax</span><span>{formatCurrency(lastSale.tax, settings.currency)}</span></div>
